...
std::vector<std::vector<Item>> getCombinationsWithRings(std::vector<Item> availableItems, std::vector<Item> weaponsAndArmor)
{
  std::vector<std::vector<Item>> combinations;
  // There's the possibility of not having rings at all, so that's the combinaton itself
...
std::vector<std::vector<Item>> getCombinationsWithArmor(std::vector<Item> availableItems, std::vector<Item> weapons)
{
  std::vector<std::vector<Item>> combinations;
  ...
  for (auto armor : availableItems)
  {
    if (armor.category == "armor")
    {
      ...
      auto ringCombinations = getCombinationsWithRings(availableItems, combination);
      for (auto combination : ringCombinations)
      {
        combinations.push_back(combination);
      }
...
std::vector<std::vector<Item>> getCombinationsWithWeapons(std::vector<Item> availableItems)
{
  std::vector<std::vector<Item>> combinations;
  for (auto weapon : availableItems)
  {
    ...
    {
      std::vector<std::vector<Item>> combination = getCombinationsWithArmor(availableItems, {weapon});
...
std::vector<std::vector<Item>> getCombinations(std::vector<Item> availableItems)
{
  return getCombinationsWithWeapons(availableItems);
}

std::tuple<int, int> playAllCombinations(std::vector<Item> availableItems, Character player, Character enemy)
{
